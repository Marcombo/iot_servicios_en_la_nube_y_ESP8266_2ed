...
#define mqtt_server "things.ubidots.com" //Broker
#define TOKEN  "*************"  //Token
...
void callback(char* tema, byte* contenido, unsigned int longitudContenido) {
  ...
  if (mensaje == "1.0") digitalWrite(pinRele, HIGH);
  else if (mensaje == "0.0") digitalWrite(pinRele, LOW); 
}
...
void setup() {
  ...
  client.ubidotsSetBroker(mqtt_server); // Asignamos el broker a utilizar
