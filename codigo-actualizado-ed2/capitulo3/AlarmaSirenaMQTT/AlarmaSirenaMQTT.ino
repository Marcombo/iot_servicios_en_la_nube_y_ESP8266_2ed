...
const char* id_cliente = "wemos_sirena";
...
unsigned int instanteActivacion;
int intervalo = 10000;
...
void callback(char* tema, byte* contenido, unsigned int longitudContenido) {
  ...
  if (mensaje == "ON"){
    ...
    releActivado = true;
    instanteActivacion = millis();
  }
}

void loop() {
  //Si se presiona el pulsador o transcurre un tiempo, se desactiva el relé
  if(releActivado){
    if(digitalRead(pinPulsador) || ((millis() - instanteActivacion) > intervalo)){
    digitalWrite(pinRele, LOW);
    releActivado = false;
    }
  }
